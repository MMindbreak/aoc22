...
let lines =
    File.ReadLines("/home/emile/recreational_programming/aoc22/src/aoc3/input")

let rucksacks: array<array<byte>> =
    lines |> Seq.map (fun s -> Encoding.ASCII.GetBytes(s)) |> Seq.toArray

let convertToValue (b: byte) =
    let res = ((int b)) - 96
    if res > 0 then res else res + 58
...
let main argv =
    let sum =
        (0, rucksacks)
        ||> Array.fold (fun acc r -> (r |> splitRucksackIntoCompartements |> findCommonItem |> convertToValue) + acc)

    let groups = Array.chunkBySize 3 rucksacks

    let commonSum =
        (0, groups)
        ||> Array.fold (fun acc g -> acc + (g |> findCommonItem |> convertToValue))
